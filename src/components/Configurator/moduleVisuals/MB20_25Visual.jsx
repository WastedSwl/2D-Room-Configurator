...
const MB20_25Visual = ({ width = 6, height = 2, label = "Nr 1 MB20-25" }) => {
  ...
  return (
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} style={{ width: "100%", height: "100%" }}>
      {/* Контур */}
      <rect x={0} y={0} width={width} height={height} fill="#eee" stroke="#222" strokeWidth={0.04} />
      {/* Штриховка */}
      <pattern id={patternId} width="0.16" height="0.16" patternTransform="rotate(45)" patternUnits="userSpaceOnUse">
        <line x1="0" y1="0" x2="0" y2="0.16" stroke="#222" strokeWidth="0.02" />
      </pattern>
      ...
      {/* Надпись */}
      <text x={width / 2} y={height / 2} textAnchor="middle" fill="#222" fontSize={0.35} fontWeight="bold" dy={-0.1}>{label}</text>
      {/* Зоны */}
